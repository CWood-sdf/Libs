...
protected:
    ...
    array<array<Num, cols>, rows> matrix;
public:
    ...
    MatrixBase() {
        matrix = array<array<Num, cols>, rows>();
        for (auto& i : matrix) {
            for (auto& x : i) {
                x = 0;
            }
    ...
    typename array<array<Num, cols>, rows>::iterator begin() {
        return matrix.begin();
    }
    typename array<array<Num, cols>, rows>::iterator end() {
        return matrix.end();
    }
};
template<typename Num, uint8_t rows, uint8_t cols>
